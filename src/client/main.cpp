...
void doCreateGroupResponse(json &responsejs){
    if (0 != responsejs["errno"].get<int>()){
        ...
        // 创建群成功
        cout << "create group success, groupid is " << responsejs["groupid"] << endl;
    }
}
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        switch (msgtype)
        {
            ...
            case LOGIN_MSG_ACK:
                // 登录成功，服务器会返回LOGIN_MSG_ACK，调用doLoginResponse
...
// 注册系统支持的客户端命令处理
// 命令处理函数都有两个参数，int表示clientfd，string表示用户输入的数据
...
void logout(int clientfd, string)
{
    ...
    {
        // 成功登出后，需要清空当前用户在客户端保存的好友列表和群组列表
